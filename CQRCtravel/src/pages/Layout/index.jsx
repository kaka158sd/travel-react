...
const Layout = () => {
  ...
  useEffect(() => {
    ...
    timerRef.current = setTimeout(() => {
      ...
      const newText = matchedTab ? matchedTab.text : '首页';
      if (newText !== text) {
        setText(newText);
      }
    }, 0);

    // 第三步：组件卸载/依赖变化时，清除定时器（核心清除逻辑）
    return () => {
      if (timerRef.current) {
        clearTimeout(timerRef.current);
  ...
  }, [location.pathname, text]); // 依赖数组保留 location.pathname + text
